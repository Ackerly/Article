# Remesh--用以开发大型复杂 Web App 的 DDD框架
## Domain-Driven Design (DDD) 是什么
...
尽管业务专家和开发团队使用同一些词汇，比如账户、订单、优惠等，但他们脑中这些词汇的含义却可能不同。很容易出现表面上大家彼此达成一致，实质上各干各的。  
...
DDD 是一种软件设计方法，它不仅仅是团队之间的沟通技巧。它必须涉及到软件开发的部分，也就是必须跟代码有关系。并且，代码必须跟领域相匹配，而领域模型则来自领域专家们。DDD 的核心是代码模型和领域模型的匹配关系，两者缺一不可。  
...
这是把战略设计和战术设计对立起来。其实它们并非互斥，并不是有两种 DDD。而是 DDD 有两个阶段：  
- 战略建模阶段，重视团队沟通和协作。这个阶段输出：Bounded Context， Context Maps 和 Ubiquitous Language
- 战术建模阶段，重视软件的开发和维护。这个阶段的输入是战略建模阶段的输出，输出的是交付的软件
...
如果 DDD 可以脱离代码，代码也无需有效地反映领域知识，翻代码找业务逻辑的意义何在呢？  
...
在《Domain Modeling Made Functional: Tackle Software Complexity with Domain-Driven Design and F#》中，作者 Scott Wlaschin 介绍了用函数式语言 F# 实现 DDD 的方式。  
其核心依据就是 —— 柯里 - 霍华德同构 (Curry–Howard isomorphism)
...
没有良好的 Sum Type 支持的语言，缺乏有效的语言特性去编码领域知识。这正是过去进二十年，OOP 里的 DDD 代码实践进展缓慢的原因之一  
...
CQRS & Event 模式，很难表达上述领域逻辑，往往需要泄露给 Consumer-side，让消费端去不断发送 Commands 以驱动领域内部发生变化。这是不合理的，领域模型不只是数据模型和通讯模型，还包括行为模型.
...
**State-Management 和 DDD 框架之间的差异**  
